...
async function processBatch(startIdx, delayMs) {
  const batch = PLAYERS.slice(startIdx, startIdx + 10);
  const rawResults = await Promise.all(batch.map(p => searchPlayer(p.name).catch(() => [])));
  
  let hitRateLimit = false;
  for (let j = 0; j < batch.length; j++) {
    const result = rawResults[j];
    if (result === 'RATE_LIMITED') {
      hitRateLimit = true;
      continue;
    }
    if (Array.isArray(result)) {
      const match = bestMatch(batch[j].name, result);
  ...
  if (hitRateLimit) {
    retries++;
    document.title = 'RL:' + retries + ' ' + completed + '/' + PLAYERS.length + ' (found ' + RESULTS.filter(r => r.fc26Found).length + ')';
    // Wait longer and retry this batch
    setTimeout(function() { processBatch(startIdx, Math.min(delayMs * 2, 10000)); }, 5000);
    return;
  }
  ...
  if (completed < PLAYERS.length) {
    // Save progress every 100 players to localStorage
    if (completed % 100 < 10) {
      localStorage.setItem('fc26Results_progress', JSON.stringify({ completed, found, results: RESULTS }));
    }
    setTimeout(function() { processBatch(startIdx + 10, 1500); }, delayMs);
  } else {
...
getVersion().then(function() {
  document.title = 'START - ver ' + VER;
  processBatch(0, 1500);
});
`

fs.writeFileSync('scripts/fc26-extractor-v4.js', js, 'utf-8')
console.log('Written v4 script: ' + js.length + ' bytes')
